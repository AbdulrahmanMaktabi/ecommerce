...
class PendingProductsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('approved', function ($query) {
                $checked = $query->is_approved == '1' ? 'checked' : '';
                return "<label class='custom-switch mt-2'>
                            <input type='checkbox' data-name='{$query->slug}' $checked name='custom-switch-checkbox' class='approved'>
                            <span class='custom-switch-indicator'></span>                        
                        </label>";
            })
            ...
            ->rawColumns(['action', 'status', 'approved',  'thumb image', 'vendor', 'category', 'child category', 'sub category'])
            ->setRowId('id');
    }
    ...
    {
        return $model->where('is_approved', false)->newQuery();
    }
